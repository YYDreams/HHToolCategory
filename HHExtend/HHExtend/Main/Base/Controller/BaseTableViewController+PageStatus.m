...
static char const * const kGoShopCarBlock  = "kGoShopCarBlock";
...
- (NSAttributedString *)descriptionForEmptyDataSet:(UIScrollView *)scrollView
{
    ...
    if (self.currentPageStatus == PageStatusError) {
      ...
      text =  @"当前暂无商品!";
    }else if (self.currentPageStatus == PageStatusGoShop){
        text = @"亲，您还没有订单哦，赶紧去购物吧!";
    }else if (self.currentPageStatus == PageStatusNotAddress) {
...
- (UIImage *)buttonImageForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state {
    ...
    if (self.currentPageStatus == PageStatusGoShop) {
        return [UIImage imageNamed:@"goShop"];
    }
    return nil;
}
...
- (void)setGoshopCarBlock:(GoShopCarBlock)goshopCarBlock{
    if (goshopCarBlock) {
        objc_setAssociatedObject(self, kGoShopCarBlock, goshopCarBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
    }
}

- (GoShopCarBlock)goshopCarBlock{
    
   
     return objc_getAssociatedObject(self, kGoShopCarBlock);
}
