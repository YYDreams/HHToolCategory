...
- (void)customizeItemWithObject:(id<YANObjectProtocol>)object{
    ...
    if ([object.icon isKindOfClass:[NSString class]]) {
        
        NSURL *url = [NSURL URLWithString:(NSString *)object.icon];
        [self.icon sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@""]];
        
    }else if ([object.icon isKindOfClass:[NSURL class]]){
        
        [self.icon sd_setImageWithURL:(NSURL *)object.icon placeholderImage:object.itemPlaceholder];
        
    }else if ([object.icon isKindOfClass:[UIImage class]]){
        
        self.icon.image = (UIImage *)object.icon;
    }
