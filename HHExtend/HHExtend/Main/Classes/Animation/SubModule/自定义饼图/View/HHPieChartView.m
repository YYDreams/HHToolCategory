...
@interface HHPieChartView()

@property(nonatomic,strong) NSMutableArray *modelArr;  // <#注释#>

@property(nonatomic,strong) NSMutableArray *colorArr;  // <#注释#>
...
-(void)drawRect:(CGRect)rect{
    ...
    if (self.dataArr.count == 0) {
        ...
        for (int i = 0; i < self.dataArr.count; i++) {
            ...
            HHPieModel *model = self.dataArr[i];
...
//                        [self addLineWithColor:color X:lineStartX Y:lineStartY name:model.name number:[NSString stringWithFormat:@"%.2f%%", model.rate * 100] radianCenter:radianCenter];
    ...
    // 在中心添加label
    HHPieCenterView *centerView = [[HHPieCenterView alloc] init];
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        // 模型数据（绘制数据时要用）
        HHPieModel *model = self.modelArr[i];
        ...
        if (i != 0) {
            ...
            if (CGRectIntersectsRect(rect, rect1)) {
                ...
                if (CGRectContainsRect(rect, rect1)) {// 包含
                    ...
                    if (i % self.dataArr.count <= self.dataArr.count * 0.5 - 1) {
                        // 将要绘制的位置在总位置偏上
...
@interface HHPieCenterView()

@property(nonatomic,strong) UIView *centerView;  // <#注释#>

@end

@implementation HHPieCenterView
